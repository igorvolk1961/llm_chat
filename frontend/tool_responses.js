    ...
    window.updateToolResponses = async function() {
        try {
            ...
            if (toolCalls.length === 0) {
                container.innerHTML = '<p style="color: #666666;">Нет активных запросов инструментов</p>';
                return;
            }
    ...
    function createToolResponseElement(toolCall, index) {
        ...
        formSection.innerHTML = `
            <div style="margin-bottom: 12px;">
                <label style="display: block; margin-bottom: 4px; color: #666666;">Tool Call ID (предзаполнено):</label>
                <input type="text" class="tool-message-id" value="${toolCall.id}" readonly style="width: 100%; padding: 8px; background-color: #f8f8f8; border: 1px solid #e0e0e0; color: #999999; border-radius: 4px;">
            </div>
            <div style="margin-bottom: 12px;">
                <label style="display: block; margin-bottom: 4px; color: #666666;">Name (предзаполнено):</label>
                <input type="text" class="tool-message-name" value="${toolCall.function?.name || ''}" readonly style="width: 100%; padding: 8px; background-color: #f8f8f8; border: 1px solid #e0e0e0; color: #999999; border-radius: 4px;">
            </div>
            <div style="margin-bottom: 12px;">
                <label style="display: block; margin-bottom: 4px; color: #666666;">Content (результат выполнения):</label>
                <textarea class="tool-message-content" placeholder='Введите результат выполнения функции (обычно JSON строка)' style="width: 100%; min-height: 120px; padding: 8px; background-color: #ffffff; border: 1px solid #d0d0d0; color: #333333; border-radius: 4px; font-family: 'Consolas', 'Monaco', monospace; resize: vertical;"></textarea>
            </div>
